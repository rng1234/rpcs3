...
void arm_interpreter::STREX(ARMv7Thread& cpu, const u32 op, const u32 cond)
{
	...
	if (ConditionPassed(cpu, cond))
	{
		...
		if (cpu.raddr != addr || cpu.rdata != data.load() || cpu.rtime != vm::reservation_acquire(addr, cpu.rtime))
		{
			// Failure
			cpu.raddr = 0;
		...
		const bool result = data.compare_and_swap_test(cpu.rdata, value);

		if (result && cpu.rdata != value)
		{
