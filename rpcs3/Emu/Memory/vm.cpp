...
namespace vm
{
	...
	void notify(u32 addr, u32 size)
	{
		...
		{
			if (ptr->addr >> 12 == addr >> 12)
			{
				if (std::memcmp(ptr->data, vm::base(ptr->addr), ptr->size) == 0)
				{
					continue;
				}

				memory_page& page = g_pages[ptr->addr >> 12];
				...
				if (ptr->stamp >= (*page.reservations)[(addr & 0xfff) >> 7].load())
				{
					continue;
				}
