...
void SPUThread::process_mfc_cmd()
{
	...
	case MFC_PUTLLC_CMD:
	{
		...
		if (raddr == ch_mfc_cmd.eal && rdata == data && vm::reservation_acquire(raddr, 128) == rtime)
		{
			if (to_write != rdata)
			{
				lv2_obj::lock_all();
	...
	case MFC_GETF_CMD:
	{
		...
		{
			// Process small transfers immediately
			if (std::shared_lock<shared_mutex> lock{vm::g_mutex, std::try_to_lock})
			{
	...
	case MFC_GETLF_CMD:
	{
		if (ch_mfc_cmd.size <= 16 && mfc_queue.size() == 0)
		{
			// TODO
		}
